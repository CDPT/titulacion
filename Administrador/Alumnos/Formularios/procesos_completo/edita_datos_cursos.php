...
$buscar="SELECT count( * ) FROM formulario_cat WHERE matricula ='$matricula' ";
if ($buscar==0){$guarda_alumno2=mysql_query("INSERT INTO formulario_cat (matricula) VALUES ('$matricula')");}
...
WHERE matricula = '$matricula'") or die("No se Puedo Guardar, Consulte1 ".mysql_error());
        ...
        if($estado=="TITULADO"){$tit=1;}
        else{$tit=0;}
            $titulado=mysql_query("UPDATE formulario SET tit='$tit' where matricula='$matricula'");
                $calficacion2=mysql_query("SELECT tit from formulario where matricula='$matricula' ");
                $filas2=mysql_fetch_array($calficacion2);
                $calificacion2=$filas2['tit'];
		...
		while ($con=mysql_fetch_array($consula_form)) {
			...
			if(empty($gr)){
				...
				WHERE matricula = '$matricula'") or die("No se Puedo Guardar, Consulte2 ".mysql_error());
						...
						$consula=mysql_query("SELECT total FROM grupos WHERE nombre_grupo='$grupo' ");
							while ($fila=mysql_fetch_array($consula)) {
									$total=$fila['total'];
							}
						$suma=$total+1;
						
							$edicion_grupo=mysql_query("UPDATE grupos SET 
			...
			if($gru==$grupo){		
				...
				$co=mysql_query("SELECT capacidad,total FROM grupos WHERE nombre_grupo='$grupo' ");
					...
					if($res <= 0 ){
					    ...
					    $error=1;
								...
								$co=mysql_query("SELECT total FROM grupos WHERE nombre_grupo='$gru' ");
										while ($fi=mysql_fetch_array($co)) {
												$to=$fi['total'];
										}
									$resta=$to-1;
