...
<table border="1">
	...
	if ($periodo==0 && $grupo==0) {
		$consula_matricula=mysql_query("SELECT * FROM formulario_cat inner join alumno on formulario_cat.matricula=alumno.matricula order by carrera, apepat") or die ("No se puede consultar la matricula 1");
	}else{
		if($periodo==0){
			$consula_matricula=mysql_query("SELECT * FROM formulario_cat inner join alumno on formulario_cat.matricula=alumno.matricula where formulario_cat.grupo='$grupo' order by carrera, apepat") or die ("No se puede consultar la matricula 2");
		}else{
			if($grupo==0){
			$consula_matricula=mysql_query("SELECT * FROM formulario_cat inner join formulario on formulario.matricula=formulario_cat.matricula inner join alumno on formulario_cat.matricula=alumno.matricula where formulario.periodo_tit='$periodo' order by carrera, apepat") or die ("No se puede consultar la matricula 3");
			$con=3;
			}else{
			$consula_matricula=mysql_query("SELECT * FROM formulario_cat inner join formulario on formulario.matricula=formulario_cat.matricula inner join alumno on formulario_cat.matricula=alumno.matricula where formulario.periodo_tit='$periodo' and formulario_cat.grupo='$grupo' order by carrera, apepat") or die ("No se puede consultar la matricula 4");
			$con=4;
			}
		}
	}
