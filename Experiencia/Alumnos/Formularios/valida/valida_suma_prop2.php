...
<script>
	$("#observa").html("<input src='../../../Imagenes/alumnos/guardar.png' type='image'/>");
	 $("#sinprop2").css({"background":"#fff","color":"#4f5baa"});	
</script>	
<?php }else{
...
$consulta1=mysql_query("SELECT COUNT(formulario.matricula) as er FROM formulario inner join formulario_er on formulario.matricula=formulario_er.matricula where (formulario.sinprop1='$prop2' or  formulario.sinprop2='$prop2') and formulario_er.submodalidad='Trabajo' and tit='0' and periodo_tit='$idper'" ) or die (mysql_error());
	$cuentadir=mysql_result($consulta1, 0);
	$n1=mysql_fetch_array($consulta1);
	echo $n1['er'];
	$ter=$n1['er'];

$consulta11=mysql_query("SELECT COUNT(formulario.matricula) as cat FROM formulario inner join formulario_cat on formulario.matricula=formulario_cat.matricula where (formulario.sinprop1='$prop2' or  formulario.sinprop2='$prop2') and tit='0' and periodo_tit='$idper'" ) or die (mysql_error());
...
$consultatot=mysql_query("SELECT COUNT(formulario.matricula) as er FROM formulario inner join formulario_er on formulario.matricula=formulario_er.matricula where (formulario.director='$prop2' or formulario.sinprop1='$prop2' or formulario.sinprop2='$prop2') and formulario_er.submodalidad='Trabajo' and tit='0' and periodo_tit='$idper' ") or die (mysql_error());
	$cuentatot=mysql_result($consultatot, 0);
	$n3=mysql_fetch_array($consultatot);
	$tTer=$n3['er'];

$consultatot1=mysql_query("SELECT COUNT(formulario.matricula) as cat FROM formulario inner join formulario_cat on formulario.matricula=formulario_cat.matricula where (formulario.director='$prop2' or formulario.sinprop1='$prop2' or formulario.sinprop2='$prop2') and tit='0' and periodo_tit='$idper' ") or die (mysql_error());
	$cuentatot1=mysql_result($consultatot1, 0);
	$n4=mysql_fetch_array($consultatot1);
	$tTcat=$n4['cat'];	
	if($idper==$periodo){
...
$dir2=$prop2."-pen";
?>
<input type="hidden" name="sinprop2" value="<?php echo $dir2; ?>" >
