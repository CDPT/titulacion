...
// cuenta director
	$consulta1=mysql_query("SELECT COUNT(formulario.matricula) FROM formulario where formulario.director='$dir' and tit='0' and periodo_tit='$idper'" ) or die (mysql_error());
	$cuentadir=mysql_result($consulta1, 0);

	if($idper==$periodo){
		$totDirAc=$cuentadir;
	// cuenta todos
		$consultatot=mysql_query("SELECT COUNT(formulario.matricula) FROM formulario where (formulario.director='$dir' or formulario.sinprop1='$dir' or formulario.sinprop2='$dir') and tit='0' and periodo_tit='$idper'") or die (mysql_error());
...
$conmaxdir=mysql_query("SELECT * FROM maximo WHERE institucion='CAT' AND nombre='Director'");
