...
<script>
	$("#observa").html("<input src='../../../Imagenes/alumnos/guardar.png' type='image'/>");
	 $("#sinsup1").css({"background":"#fff","color":"#4f5baa"});	
</script>	
<?php }else{
	if ($sup1==$dir || $sup1==$prop1 || $sup1==$prop2 || $sup1==$sup2){ ?>
<script>
		$("#observa").html("");	
...
$consulta1=mysql_query("SELECT COUNT(sinsup1) FROM formulario WHERE sinsup1='$sup1' and periodo_tit='$periodo'  ");
	$fila=mysql_fetch_array($consulta1);
		$cuenta=$fila['COUNT(sinsup1)'];
...
$dir2=$sup1."-pen";
?>
<input type="hidden" name="sinsup1" value="<?php echo $dir2; ?>" >
