	...
	<tr>
		...
		<td>
			<select name="periodo">	
				...
				<?php
				$consula_periodo=mysql_query("SELECT * FROM periodo_cat WHERE estado='1'");
		...
		<td>
			<select name="grupo">	
				<option value="0"></option>
				<?php
				$consula_grupo=mysql_query("SELECT * FROM grupos_cat  WHERE estado='1'");
				while($fila=mysql_fetch_array($consula_grupo)){
				$nombre_grupo=$fila['nombre_grupo'];
				?>
				<option><?php echo $nombre_grupo; ?></option>
				<?php } ?>
			</select>
		</td>
	</tr>
